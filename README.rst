...
Sendprism relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'Sendprism' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "Sendprism <noreply@example.com>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "Sendprism <noreply@example.com>" 
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[Sendprism] "
======================================= =========================== ============================================== ======================================================================
...
To run the tests, enter the sendprism/tests directory and run the following commands::
...
..  code-block:: bash
    ...
    git remote add dokku dokku@yourservername.com:sendprism
    git push dokku master
    ssh -t dokku@yourservername.com dokku redis:create sendprism-redis
    ssh -t dokku@yourservername.com dokku redis:link sendprism-redis sendprism
    ssh -t dokku@yourservername.com dokku postgres:create sendprism-postgres
    ssh -t dokku@yourservername.com dokku postgres:link sendprism-postgres sendprism
    ssh -t dokku@yourservername.com dokku config:set sendprism DJANGO_SECRET_KEY=RANDOM_SECRET_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set sendprism DJANGO_SETTINGS_MODULE='config.settings.production'
    ssh -t dokku@yourservername.com dokku config:set sendprism DJANGO_AWS_ACCESS_KEY_ID=YOUR_AWS_ID_HERE
    ssh -t dokku@yourservername.com dokku config:set sendprism DJANGO_AWS_SECRET_ACCESS_KEY=YOUR_AWS_SECRET_ACCESS_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set sendprism DJANGO_AWS_STORAGE_BUCKET_NAME=YOUR_AWS_S3_BUCKET_NAME_HERE
    ssh -t dokku@yourservername.com dokku config:set sendprism DJANGO_MAILGUN_API_KEY=YOUR_MAILGUN_API_KEY
    ssh -t dokku@yourservername.com dokku config:set sendprism DJANGO_MAILGUN_SERVER_NAME=YOUR_MAILGUN_SERVER
    ssh -t dokku@yourservername.com dokku run sendprism python manage.py migrate
    ssh -t dokku@yourservername.com dokku run sendprism python manage.py createsuperuser
...
Postgres is saving its database files to `/data/sendprism/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/sendprism.conf` and run::

    supervisorctl reread
    supervisorctl start sendprism
